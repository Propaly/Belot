...
## История, replay viewer и преглед на точки

Сървърът пази всяка игра в **SQLite** (`belot-history.db`, вграденото `node:sqlite`). В приложението бутонът **🕘 История** (началният екран или хедъра) показва списък със записаните игри и пълноценен **replay viewer**:

- стъпване напред/назад през всяко действие (обяви и карти), с автоматично преиграване;
- **timeline** със слайдер за превъртане и маркери за **акценти** (договор, анонси терца/кварта/квинта/каре, контра/реконтра, капо, край на раздаване), плюс бутони „акцент ⇤/⇥“ за прескачане между тях;
- **браузър на стари игри**: филтър по период (7 дни / 30 дни / всички), групиране по дата и „Още по-стари игри“ (пагинация), така че да се стига и до игри отпреди седмици;
- четирите ръце face-up, коз/договор, контра, точки;
- панел „Раздавания · точки и анонси“ - за всяко раздаване: договор, декларатор, точки от взятки, резултат за раздаването и **анонсите (терца/кварта/квинта, каре)** по играчи.

Двете API точки (на същия порт като играта):

```bash
curl http://localhost:8787/api/history        # списък с играните игри
curl http://localhost:8787/api/history/1      # игра + нотация за replay
```

Други промени:

- **Имената вече не се разместват** - сървърът държи имената по седалки и ги презаписва преди всяко излъчване (всяко ново раздаване/4 паса създава нови играчи с подразбиращи се имена).
- **Ботове запълват празните седалки - но само след като стаята стартира** (онлайн). Играта не се изиграва сама, докато хората не влязат; ботовете се включват едва когато всичките 4 седалки са заети (или е получен явен `start`), и поемат седалка само ако тя остане без клиент (напр. някой излезе), за да не блокира.
- **Преиграване на последната взятка** - бутон ▶ в играта показва отново току-що изиграната взятка.

```bash
npm run test:history   # GameRecorder -> нотация -> replay + SQLite + преглед на анонси
npm run test:online    # онлайн с 1 клиент + ботове: без hang, стабилни имена, записана игра
```
